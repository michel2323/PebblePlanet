...
GColor enamel_get_HourColor(){
	Tuple* tuple = dict_find(&s_dict, 683434074);
...
GColor enamel_get_MinuteColor(){
	Tuple* tuple = dict_find(&s_dict, 2175957230);
...
GColor enamel_get_DateColor(){
	Tuple* tuple = dict_find(&s_dict, 2796082000);
	return tuple ? GColorFromHEX(tuple->value->int32) : GColorFromHEX(0xFFFFFF);
}
// -----------------------------------------------------
...
static uint32_t prv_map_messagekey(const uint32_t key){
	if( key == MESSAGE_KEY_HourColor) return 683434074;
	if( key == MESSAGE_KEY_MinuteColor) return 2175957230;
	if( key == MESSAGE_KEY_DateColor) return 2796082000;
	return 0;
}
...
static void prv_inbox_received_handle(DictionaryIterator *iter, void *context) {
	if(dict_find(iter, MESSAGE_KEY_HourColor)){
		if(s_dict_buffer){
			free(s_dict_buffer);
			s_dict_buffer = NULL;
		}
